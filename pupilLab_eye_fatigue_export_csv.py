...
class ConvEyeFatigue2csv:
    ...
    def pldata2dataframe(self, columns, method_is_3d):
        ...
        #print('eye type in func =', self.eyeType)
...
# ファイルブラウザを使用したファイル入力
layout = [[sg.Text('.pldataファイルの入力')],
          [sg.InputText(key='INPUT_FILE', enable_events=True, size=(45,1)),sg.FileBrowse('参照', file_types=(('pldataファイル', '*.pldata'),))],
          [sg.Text('計測単眼選択')],
          [sg.Radio('左目', group_id='radioint',key='EYE_LEFT'),sg.Radio('右目', group_id='radioint',key='EYE_RIGHT',default=True)],
          [sg.Text('グラフ表示'),sg.Button('Display',key='-display-')],
          [sg.Text('出力：', key='-dis_status-')],
          [sg.Text('------------------------------------ 結果出力 ------------------------------------')],
          [sg.Checkbox('グラフ画像の出力', key='SAVE_GRAPH', default=True,)],
          [sg.InputText(key='OUTPUT_IMG_FILE', enable_events=True, size=(45,1)),
           sg.FileSaveAs('参照', file_types=(('pngファイル', '*.png'),))],
          [sg.Checkbox('csvファイルの出力', key='SAVE_CSV',default=True)],
          [sg.InputText(key='OUTPUT_CSV_FILE', enable_events=True, size=(45,1)),
           sg.FileSaveAs('参照', file_types=(('csvファイル', '*.csv'),))],
          [sg.Button('保存',key='-save-'),sg.Text('出力：',key='-output-')],
          [sg.Exit()]
         ]
...
while True:
    ...
    if event == '-save-':
        ...
        window['-output-'].update(f'Data of Figrue {Disp_cnt}: ファイル出力完了')
